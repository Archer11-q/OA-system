...
| 📅 **日程管理** | CRUD + 日期范围查询 + 参与人 + 提醒通知 + 三级可见性 + 日历视图 |
| 💰 **报销管理** | CRUD + 按状态过滤 + 金额统计 + 审批联动 + 附件上传 |
...
| Phase 5 | 上线准备（MySQL 切换 + 单元测试 + Docker + 接口文档） | ✅ |
...
| ✅ | 日程日历视图 | FullCalendar 对接，日/周/月视图（DEV-31） |
| ✅ | 前端优化 | 路由懒加载、keep-alive 缓存、构建分包（DEV-32） |
| ✅ | 接口文档 | Knife4j @Parameter + @Schema 全覆盖（DEV-33） |
| ✅ | 报销附件 | 附件上传与报销关联（DEV-34） |
| ✅ | 部门统计 | 各部门用户数量（含子部门汇总）（DEV-35） |
| ✅ | 日程提醒 | 提前提醒 + 看板倒计时展示（DEV-36） |
| ✅ | 可见性规则 | 私有/部门/公开三级可见性（DEV-37） |
