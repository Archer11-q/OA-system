...
- DEV-03：实现角色管理（CRUD）与菜单权限分配，完成 RBAC 的数据库端实现。
 ...
 - DEV-05：已实现权限注入（UserDetailsService），在 JWT 认证中加载用户权限并完善 `SecurityUtils`。
  - DEV-06：已实现菜单管理 CRUD（新增/删除）与方法级权限注解，完善接口权限控制。
  - DEV-07：已实现部门管理 CRUD（新增/更新/删除/查询树），并为部门写操作添加管理员权限校验。




 - DEV-08：已实现考勤模块基础（签到/签退/按月查询/请假申请），包含实体、Mapper、Service 与 Controller。
 - DEV-09：已实现公告通知模块基础（公告列表/详情/发布/编辑/删除），并为发布/编辑/删除接口添加管理员权限校验。
